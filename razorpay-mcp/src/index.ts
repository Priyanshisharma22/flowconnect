...
const server = new Server(
  { name: "razorpay-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
  ...
  try {
    switch (name) {
      ...
      case "get_payments": {
        ...
        const payments = data.items.map((p: any) => ({
          id:      p.id,
      ...
      case "get_payment_summary": {
        ...
        const failed   = items.filter((p: any) => p.status === "failed");
        const totalAmt = captured.reduce((s: number, p: any) => s + p.amount, 0);
        const methods: Record<string, number> = {};
        items.forEach((p: any) => { methods[p.method] = (methods[p.method] || 0) + 1; });
        return {
          content: [{
            type: "text",
            text: JSON.stringify({
      ...
      case "get_total_revenue": {
        ...
        const total    = captured.reduce((s: number, p: any) => s + p.amount, 0);
        return {
          content: [{
            type: "text",
            text: JSON.stringify({
      ...
      case "create_refund": {
        ...
        if (args?.reason) body.notes  = { reason: args.reason };
      ...
      case "get_orders": {
        ...
        return {
          content: [{
            type: "text",
            text: JSON.stringify(data.items.map((o: any) => ({
              id:      o.id,
